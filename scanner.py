...
import re                          # for validating the port range
...
class PortScannerArgs:
    ...
    def __init__(self):
        ...
        self.__parser.add_argument("-t", "--threads", help="The number of threads to use for the port scans (default: 1, max: 100)", type=int, default=100)
    ...
    def parse_args(self):
        ...
        if args.port_range:
            try:
                start_port, end_port = self.__parse_port_range(args.port_range)
                args.start_port = start_port
                args.end_port = end_port
            except argparse.ArgumentTypeError as e:
                self.__parser.error(str(e))
        else:
            args.start_port = min(TOP_VULN_PORTS)
            args.end_port = max(TOP_VULN_PORTS)
        return args
    
    def __parse_port_range(self, port_range_str):
        # Use regular expressions to parse the port range string
        port_range_regex = r'^(\d{1,5})-(\d{1,5})$'
        match = re.match(port_range_regex, port_range_str)
        if match:
            start_port = int(match.group(1))
            end_port = int(match.group(2))
            # Make sure start_port <= end_port
            if start_port > end_port:
                start_port, end_port = end_port, start_port
            # Make sure start_port >= 1 and end_port <= 65535
            start_port = max(start_port, 1)
            end_port = min(end_port, 65535)
        else:
            raise argparse.ArgumentTypeError(f"Invalid port range: {port_range_str}. Port range should be in the format 'start_port-end_port', where start_port and end_port are integers between 1 and 65535.")
